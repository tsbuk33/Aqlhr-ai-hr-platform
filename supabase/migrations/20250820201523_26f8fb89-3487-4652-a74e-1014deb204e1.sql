-- Corporate Culture Intelligence (CCI) Database Schema - Final
...
-- 7. Quality Control Materialized View (FIXED - qualified column names)
CREATE MATERIALIZED VIEW public.cci_response_quality_v1 AS
SELECT 
  r.survey_id,
  w.id as wave_id,
  COUNT(*) as total_responses,
  COUNT(*) FILTER (WHERE r.is_flagged = true) as flagged_responses,
  ROUND(
    COUNT(*) FILTER (WHERE r.is_flagged = true)::numeric / 
    NULLIF(COUNT(*), 0) * 100, 2
  ) as flagged_percentage,
  COUNT(*) FILTER (WHERE r.duration_seconds < 120) as too_fast_responses,
  AVG(r.duration_seconds) as avg_duration_seconds
FROM public.cci_responses r
LEFT JOIN public.cci_waves w ON w.survey_id = r.survey_id
GROUP BY r.survey_id, w.id;
