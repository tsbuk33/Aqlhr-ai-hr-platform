...
-- Function to run daily compliance for all companies (called by cron)
CREATE OR REPLACE FUNCTION public.compliance_daily_cron_job()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Schedule the daily compliance cron job at 02:00 KSA (23:00 UTC)
SELECT cron.schedule(
  'daily-compliance-autopilot',
  '0 23 * * *', -- Daily at 23:00 UTC (02:00 KSA)
  'SELECT public.compliance_daily_cron_job();'
);

-- Grant necessary permissions
GRANT EXECUTE ON FUNCTION public.compliance_run_now_v1(uuid, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.compliance_daily_cron_job() TO postgres;
