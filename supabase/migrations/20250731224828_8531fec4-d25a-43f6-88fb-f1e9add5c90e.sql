...
-- 3) PUBLIC POLICIES FOR SAUDI REFERENCE TABLES
CREATE POLICY IF NOT EXISTS saudi_regions_public_select
  ON public.saudi_regions FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS saudi_cities_public_select
...
-- 6) MANUAL VERIFICATION STEPS (in comments)
--   • Check owner: SELECT proname, proowner::regrole FROM pg_proc WHERE proname='get_user_company_id';
--   • If needed: ALTER FUNCTION public.get_user_company_id() OWNER TO postgres;
--   • Test multi-tenant isolation by logging in as different users.
