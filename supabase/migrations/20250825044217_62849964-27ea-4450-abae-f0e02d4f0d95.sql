-- Fix retention SQL functions that are trying to use avg() on text fields
-- Update retention_overview_v1 to cast text fields to numeric for averaging
...
AS $function$
  with base_data as (
    select 
      ...
      -- Use coalesce and safe casting for salary fields
      coalesce(
        case when e.basic_salary ~ '^[0-9]+\.?[0-9]*$' 
...
-- Update retention_drivers_v1 to handle text field casting properly
