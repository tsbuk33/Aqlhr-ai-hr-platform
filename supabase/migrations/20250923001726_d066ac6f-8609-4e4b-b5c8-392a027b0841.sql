-- Manager Dashboard Backend Infrastructure (Skip existing tables)
...
-- Action items and recommendations (skip if exists)
...
-- Enable RLS on action_items if it was created
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'action_items' AND policyname = 'Company users can view action items') THEN
...
-- Action items policies (conditional)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'action_items' AND policyname = 'Company users can view action items') THEN
...
-- Create conditional index for action_items
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'action_items' AND indexname = 'idx_action_items_manager_id') THEN
...
-- Create conditional trigger for action_items
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_action_items_updated_at') THEN
