-- Create OSI layers view based on grades
CREATE OR REPLACE VIEW osi_layers_by_grade_v1 AS
WITH grade_layer_mapping AS (
  SELECT 
    5 as grade, 'exec' as layer_code, 1 as layer_order, 'Executive' as name_en, 'تنفيذي' as name_ar, 30 as target_rate
  UNION ALL SELECT 
    4, 'manager', 2, 'Manager', 'مدير', 40
  UNION ALL SELECT 
    3, 'supervisor', 3, 'Supervisor', 'مشرف', 50
  UNION ALL SELECT 
    2, 'professional', 4, 'Professional', 'محترف', 60
  UNION ALL SELECT 
    1, 'staff', 5, 'Staff', 'موظف', 70
),
layer_stats AS (
  SELECT 
    e.company_id,
    COALESCE(e.grade, 1) as grade,
    COUNT(*) as headcount,
  ...
  GROUP BY e.company_id, COALESCE(e.grade, 1)
)
SELECT 
  ls.company_id,
  glm.layer_code,
  ...
  glm.target_rate
FROM layer_stats ls
JOIN grade_layer_mapping glm ON ls.grade = glm.grade;
...
AS $function$
BEGIN
  -- Ensure all employees have grades 1-5
  UPDATE hr_employees 
  SET grade = CASE 
    WHEN grade IS NULL THEN 
      CASE 
        WHEN EXTRACT(MONTH FROM hire_date) % 5 = 0 THEN 5
        WHEN EXTRACT(MONTH FROM hire_date) % 5 = 1 THEN 4
        WHEN EXTRACT(MONTH FROM hire_date) % 5 = 2 THEN 3
        WHEN EXTRACT(MONTH FROM hire_date) % 5 = 3 THEN 2
        ELSE 1
      END
    ELSE grade
  END
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hr_employees_company_grade 
ON hr_employees(company_id, grade) 
