...
USING (
  ...
  EXISTS (
    ...
    AND ur.role IN ('admin'::app_role, 'super_admin'::app_role, 'hr_manager'::app_role)
  )
);
...
EXCEPTION WHEN OTHERS THEN
  -- Fail silently to not block access but log error
  NULL;
END;
$$;
...
-- Only HR managers and admins can INSERT medical data
...
-- Only HR managers and admins can UPDATE medical data
...
BEGIN
  -- Log access attempt
  PERFORM public.log_medical_access(p_employee_id, 'MASKED_VIEW', NULL, 'Data masking function called');
