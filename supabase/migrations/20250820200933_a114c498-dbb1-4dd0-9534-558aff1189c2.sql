-- Corporate Culture Intelligence (CCI) Database Schema
...
-- Get user tenant function for RLS
...
-- CCI Responses policies (restricted access)
...
SELECT 
  ...
  CASE 
    WHEN s.anonymity_min_n IS NULL OR cci_scores.n >= s.anonymity_min_n 
    THEN barrett ELSE NULL 
  END as barrett,
  CASE 
    WHEN s.anonymity_min_n IS NULL OR cci_scores.n >= s.anonymity_min_n 
    THEN psych_safety ELSE NULL 
  END as psych_safety,
  CASE 
    WHEN s.anonymity_min_n IS NULL OR cci_scores.n >= s.anonymity_min_n 
    THEN risk_index ELSE NULL 
  END as risk_index,
  CASE 
    WHEN s.anonymity_min_n IS NULL OR cci_scores.n >= s.anonymity_min_n 
    THEN balance_score ELSE NULL 
  END as balance_score,
  CASE 
    WHEN s.anonymity_min_n IS NULL OR cci_scores.n >= s.anonymity_min_n 
    THEN values_tags ELSE NULL 
  END as values_tags,
  n,
  last_computed_at,
  created_at
...
CREATE TRIGGER cci_response_quality_check
  ...
  EXECUTE FUNCTION (
    CASE 
      WHEN NOT public.cci_is_response_valid(NEW.answers, NEW.duration_seconds)
      THEN NEW.is_flagged = true, NEW.flag_reason = 'Quality check failed'
      ELSE NEW
    END
  );
