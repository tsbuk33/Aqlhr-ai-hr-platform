-- Fix gender values in dev_seed_employees_v1
...
begin
  ...
  -- Insert jobs using title_en, title_ar
  ...
  -- Generate employees with correct gender values ('M' or 'F')
  for v_i in 1..p_n loop
    insert into public.hr_employees (
      ...
      case when random() < 0.95 then 'active' else 'terminated' end,
      random() < 0.65,
      (array['SA','IN','PK','BD','EG','JO','LB','SY','YE','SD','PH','NP','LK','US','GB'])[1 + (v_i % 15)],
      case when random() < 0.7 then 'M' else 'F' end, -- Fixed: use 'M'/'F' instead of 'male'/'female'
