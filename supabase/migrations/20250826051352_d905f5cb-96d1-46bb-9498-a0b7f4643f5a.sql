-- CRITICAL SECURITY FIX: Employee Personal Information Access Control
...
-- Update the audit function to include hr_employees risk assessment
CREATE OR REPLACE FUNCTION public.audit_employee_data_exposure_risk()
RETURNS TABLE(
  table_name text,
  risk_level text,
...
BEGIN
  ...
  SELECT 
    ...
    ('Contains highly sensitive employee PII and financial data for ' || COUNT(*) || ' employees')::text,
    'Ensure role-based access is properly configured and regularly audited'::text,
    COUNT(*)::bigint
  FROM public.hr_employees
  ...
  SELECT 
    'hr_employees_policies'::text,
    CASE 
      WHEN COUNT(*) < 4 THEN 'HIGH'
      WHEN COUNT(*) < 6 THEN 'MEDIUM'
      ELSE 'LOW'
    END::text,
    ('Only ' || COUNT(*) || ' RLS policies found - ensure comprehensive coverage')::text,
    'Implement separate policies for SELECT, INSERT, UPDATE, DELETE with role-based restrictions'::text,
    COUNT(*)::bigint
  FROM pg_policies 
...
BEGIN
  ...
  RAISE NOTICE '5. Enhanced audit functions for ongoing monitoring';
  RAISE NOTICE '6. Restricted financial and personal data to HR personnel only';
END $$;