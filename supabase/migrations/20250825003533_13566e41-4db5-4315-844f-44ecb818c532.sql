-- Fix retention_drivers_v1 function to handle potential text values in JSONB
...
BEGIN
  ...
  WITH driver_analysis AS (
    SELECT 'Compensation' as driver, COUNT(*) as count,
           AVG(CASE 
             WHEN rf.features->>'risk_score' ~ '^[0-9\.]+$' 
             THEN (rf.features->>'risk_score')::numeric 
             ELSE 0 
           END) * 100 as avg_risk
    ...
    WHERE rf.tenant_id = p_tenant 
      ...
      AND rf.features->>'salary_percentile' ~ '^[0-9\.]+$'
    ...
    WHERE rf.tenant_id = p_tenant 
      AND e.employment_status = 'active'
      AND rf.features->>'tenure_months' ~ '^[0-9\.]+$'
    ...
    WHERE rf.tenant_id = p_tenant 
      AND e.employment_status = 'active'
      AND rf.features->>'performance_rating' ~ '^[0-9\.]+$'
      AND (rf.features->>'performance_rating')::numeric >= 4.0
      AND rf.features->>'risk_score' ~ '^[0-9\.]+$'
  ...
  SELECT 
    da.driver,
    CASE 
      WHEN tar.total > 0 THEN ROUND((da.count * 100.0 / tar.total)::numeric, 1)
      ELSE 0
    END,
    da.count::integer
