-- OSI v1: Org Structure Intelligence
...
-- Views for org structure analysis
...
LEFT JOIN public.hr_jobs j ON e.job_id = j.id
...
manager_info AS (
  SELECT 
    ...
    mc.avg_team_cost,
    COALESCE(b.target_span, 7) as target_span -- default benchmark
  FROM public.hr_employees e
  LEFT JOIN public.hr_grades g ON e.grade_id = g.id
  LEFT JOIN manager_counts mc ON e.id = mc.manager_id
  LEFT JOIN public.osi_benchmarks b ON g.name_en = b.grade
...
SELECT 
  ...
  COALESCE(e.base_salary + e.allowances, 
    CASE 
      WHEN g.name_en ILIKE '%senior%' THEN 15000
      WHEN g.name_en ILIKE '%manager%' THEN 12000
      WHEN g.name_en ILIKE '%specialist%' THEN 8000
      ELSE 6000
    END
  ) as monthly_cost
FROM public.hr_employees e
...
BEGIN
  ...
  SELECT 
    ...
    MAX(layer),
    ROUND(AVG(CASE WHEN is_saudi THEN 100.0 ELSE 0.0 END), 1),
  ...
  SELECT ROUND(
    (SUM(monthly_cost) FILTER (WHERE manager_id IS NOT NULL) / 
     NULLIF(SUM(monthly_cost), 0)) * 100, 2
  ) INTO v_cost_to_manage
  FROM public.v_org_current oc
...
BEGIN
  ...
  IF p_dim = 'department' THEN
    ...
    SELECT 
      ...
      ROUND(AVG(ms.span), 2) as span_avg,
      MAX(ol.layer) as layers,
    ...
    LEFT JOIN public.hr_departments d ON e.department_id = d.id
