-- 1) Create view to handle reverse scoring and extract Likert responses (proper lateral join)
...
join public.cci_survey_items i on i.id = (regexp_replace(kv.key, '^item::', ''))::uuid and i.survey_id = r.survey_id
...
begin
  ...
  values_alignment as (
    ...
    where tenant_id = p_tenant 
      ...
      and wave_id = p_wave 
      and is_flagged = false
      and r.answers ? 'values_current'
      and r.answers ? 'values_desired'
    limit 1
  )
