...
-- Use existing gov_adapters table structure and add missing columns
alter table if exists public.gov_adapters add column if not exists config jsonb default '{}'::jsonb;
alter table if exists public.gov_adapters add column if not exists last_error text;
...
-- Helper RPCs using existing column names
create or replace function public.gov_get_status_v1(p_tenant uuid)
returns table(system text, status text, last_sync timestamptz, last_error text)
language sql stable security definer set search_path=public as $$
  select system, status, last_sync, last_error
