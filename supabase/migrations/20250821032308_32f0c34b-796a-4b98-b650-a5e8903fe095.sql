-- Create KPI snapshots table for historical data
CREATE TABLE public.kpi_snapshots (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  company_id uuid NOT NULL,
  snap_date date NOT NULL,
  total_employees numeric DEFAULT 0,
  saudization_rate numeric DEFAULT 0,
  hse_safety_score numeric DEFAULT 0,
  active_users numeric DEFAULT 0,
  docs_processed numeric DEFAULT 0,
  training_hours numeric DEFAULT 0,
  compliance_score numeric DEFAULT 0,
  talent_pipeline_strength numeric DEFAULT 0,
  predictive_risk_high numeric DEFAULT 0,
  employee_experience_10 numeric DEFAULT 0,
  workforce_forecast_accuracy numeric DEFAULT 0,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  updated_at timestamp with time zone NOT NULL DEFAULT now()
);

-- Enable RLS on KPI snapshots
ALTER TABLE public.kpi_snapshots ENABLE ROW LEVEL SECURITY;

-- Create policy for company access
CREATE POLICY "Users can manage KPI snapshots from their company" 
ON public.kpi_snapshots 
FOR ALL 
USING (company_id = get_user_company_id())
WITH CHECK (company_id = get_user_company_id());

-- Create unique index to prevent duplicate snapshots per day
CREATE UNIQUE INDEX idx_kpi_snapshots_company_date 
