-- Create diagnostic overview view for cross-signals analysis
...
osi_latest AS (
  SELECT 
    ...
    highest_saudi_layer,
    critical_layers,
    layers_meeting_target,
    span_outliers_low + span_outliers_high as total_span_outliers,
    management_cost
  FROM public.osi_layers_mv_v1 
  GROUP BY tenant_id, total_layers, highest_saudi_layer, critical_layers, layers_meeting_target, span_outliers_low, span_outliers_high, management_cost
),
retention_latest AS (
  SELECT 
    tenant_id,
...
SELECT 
  COALESCE(c.tenant_id, o.tenant_id, r.tenant_id, comp.tenant_id) as tenant_id,
  ...
  -- OSI Metrics
  COALESCE(o.total_layers, 0) as osi_total_layers,
  COALESCE(o.highest_saudi_layer, 0) as osi_highest_saudi_layer,
  COALESCE(o.critical_layers, 0) as osi_critical_layers,
  COALESCE(o.layers_meeting_target, 0) as osi_layers_meeting_target,
  COALESCE(o.total_span_outliers, 0) as osi_span_outliers,
  COALESCE(o.management_cost, 0) as osi_management_cost,
  ...
  CASE 
    ...
    WHEN COALESCE(c.balance_score, 0) < 70 OR COALESCE(o.critical_layers, 0) > 2 OR comp.nitaqat_status = 'yellow' THEN 'warning'
    ELSE 'healthy'
...
FROM cci_latest c
FULL OUTER JOIN osi_latest o ON c.tenant_id = o.tenant_id  
FULL OUTER JOIN retention_latest r ON COALESCE(c.tenant_id, o.tenant_id) = r.tenant_id
FULL OUTER JOIN compliance_latest comp ON COALESCE(c.tenant_id, o.tenant_id, r.tenant_id) = comp.tenant_id
WHERE COALESCE(c.tenant_id, o.tenant_id, r.tenant_id, comp.tenant_id) IS NOT NULL;

-- Enable RLS on the view
ALTER VIEW public.diagnostic_overview_v1 SET (security_barrier = true);
