...
-- HSE incidents table (using company_id for consistency)
...
-- Integrations table (using company_id for consistency)
...
-- Docs events table (using company_id for consistency)
...
-- HR training table (using company_id for consistency)
...
RETURNS NUMERIC LANGUAGE SQL STABLE AS $$
  WITH i AS (
    ...
    FROM public.integrations WHERE company_id = p_tenant
  ),
  d AS (
...
hse7 AS (
  ...
  WHERE company_id = p_tenant AND occurred_at > CURRENT_DATE - INTERVAL '7 days'
),
hse90 AS (
  SELECT GREATEST(count(*)::NUMERIC, 1) as n90 FROM public.hse_incidents
  WHERE company_id = p_tenant AND occurred_at > CURRENT_DATE - INTERVAL '90 days'
)
SELECT * FROM (
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
      AND tablename = 'hse_incidents' 
      AND policyname = 'tenant_isolation_hse_incidents'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
      ...
      AND policyname = 'tenant_isolation_integrations'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
      AND tablename = 'docs_events' 
      AND policyname = 'tenant_isolation_docs_events'
  ) THEN
