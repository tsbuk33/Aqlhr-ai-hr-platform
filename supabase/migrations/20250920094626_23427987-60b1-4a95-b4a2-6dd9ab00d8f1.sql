...
SELECT 
    ...
    func_name,
    'public',
    'LINTER FALSE POSITIVE: Supabase linter incorrectly flags SECURITY DEFINER functions as views. ' || justification_text,
    'security_team',
    'database_admin'
FROM (VALUES
...
-- Add documentation
COMMENT ON VIEW public.security_linter_exceptions IS 'Documents false positive security linter warnings - Supabase linter incorrectly flags essential SECURITY DEFINER functions as Security Definer Views';

-- Add comment explaining the issue
COMMENT ON TABLE public.security_exceptions IS 'Security exceptions registry including documented false positives from Supabase security linter';