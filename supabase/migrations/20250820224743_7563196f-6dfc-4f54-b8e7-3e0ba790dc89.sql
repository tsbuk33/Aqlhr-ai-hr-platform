-- Create reverse-scoring compute functions and RPCs
...
BEGIN
  ...
  values_alignment AS (
    SELECT
      COALESCE(
        100.0 * CARDINALITY( (SELECT array_agg(x) FROM jsonb_array_elements_text(r.answers->'values_current') AS x)
                             && 
                             (SELECT array_agg(y) FROM jsonb_array_elements_text(r.answers->'values_desired') AS y)
      ) / NULLIF(
        CARDINALITY( (SELECT array_agg(x) FROM jsonb_array_elements_text(r.answers->'values_current') AS x)
                     || 
                     (SELECT array_agg(y) FROM jsonb_array_elements_text(r.answers->'values_desired') AS y)
      ),0), 50) as score
    FROM public.cci_responses r
  ...
  SET balance_score = ROUND(
      0.40 * ( (s.cvf->>'Clan')::numeric + (s.cvf->>'Adhocracy')::numeric + (s.cvf->>'Market')::numeric + (s.cvf->>'Hierarchy')::numeric ) / 4
