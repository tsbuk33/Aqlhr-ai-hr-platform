...
create policy if not exists "tenant_rw_adapters" on public.gov_adapters
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());

create policy if not exists "tenant_rw_jobs" on public.gov_sync_jobs
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());

create policy if not exists "tenant_rw_events" on public.gov_events
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());

create policy if not exists "tenant_rw_docs" on public.gov_documents
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());
...
-- Helper function to get demo tenant ID (if needed)
create or replace function public.get_demo_tenant_id()
returns uuid
language sql stable security definer as $$
  select coalesce(public.get_user_company_id(), '00000000-0000-0000-0000-000000000000'::uuid);
$$;