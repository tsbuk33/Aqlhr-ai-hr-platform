-- SECURITY DEFINER ANALYSIS AND DOCUMENTATION
...
-- Create a view for approved security exceptions (this addresses the "Security Definer View" concern)
-- This view will be SECURITY INVOKER (default) and rely on RLS policies
...
COMMENT ON TABLE public.security_exceptions IS 'Documents and tracks approved security exceptions including SECURITY DEFINER functions with business justifications';
COMMENT ON VIEW public.approved_security_functions IS 'Read-only view of approved SECURITY DEFINER functions with justifications - uses SECURITY INVOKER';
COMMENT ON VIEW public.rls_coverage_report IS 'Security metrics dashboard view - uses SECURITY INVOKER instead of SECURITY DEFINER for better security compliance';