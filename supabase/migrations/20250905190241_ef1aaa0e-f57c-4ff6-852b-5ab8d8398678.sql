...
CREATE INDEX IF NOT EXISTS idx_automation_metrics_tenant_time 
ON public.automation_metrics(tenant_id, execution_time DESC);

CREATE INDEX IF NOT EXISTS idx_automation_metrics_success 
ON public.automation_metrics(tenant_id, success, execution_time DESC);

-- Create function to get automation metrics overview
...
BEGIN
  ...
  SELECT 
    m.total,
    m.successful,
    CASE WHEN m.total > 0 THEN ROUND((m.successful::NUMERIC / m.total::NUMERIC) * 100, 2) ELSE 0 END,
