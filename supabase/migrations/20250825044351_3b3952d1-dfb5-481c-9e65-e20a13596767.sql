-- Drop existing retention functions that have avg(text) issues
DROP FUNCTION IF EXISTS public.retention_overview_v1(uuid);
DROP FUNCTION IF EXISTS public.retention_drivers_v1(uuid);

-- Recreate retention_overview_v1 with proper text-to-numeric casting
...
AS $function$
  with base_data as (
    select 
      ...
      coalesce(
        case when e.basic_salary::text ~ '^[0-9]+\.?[0-9]*$' 
             then e.basic_salary::numeric 
             else 0 end, 0
      ) as salary_numeric
...
-- Recreate retention_drivers_v1 with proper text-to-numeric casting
