...
create policy if not exists "tenant_rw_sessions" on public.assistant_sessions
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());
create policy if not exists "tenant_rw_messages" on public.assistant_messages
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());
create policy if not exists "tenant_rw_tool_logs" on public.assistant_tool_logs
  for all using (tenant_id = public.get_user_company_id()) with check (tenant_id = public.get_user_company_id());
