...
-- 2) Recreate alerts with correct HSE date and demo alert guard
...
hse7 AS (
  SELECT COUNT(*)::numeric as n7
  FROM public.hse_incidents
  WHERE company_id = p_tenant
    AND COALESCE(occurred_at, created_at) > current_date - interval '7 days'
),
hse90 AS (
  SELECT GREATEST(COUNT(*)::numeric, 1) as n90
  FROM public.hse_incidents
  WHERE company_id = p_tenant
    AND COALESCE(occurred_at, created_at) > current_date - interval '90 days'
)
SELECT * FROM (
  -- Saudization risk
  ...
  -- HSE spike
  SELECT 'high'::text, 'hse'::text,
         'HSE incidents spiked in the last 7 days'::text,
         'ارتفاع ملحوظ في الحوادث خلال 7 أيام'::text,
         (SELECT n7 FROM hse7),
         null::numeric,
         'Immediate toolbox talks + site audit this week.'::text,
         'تنفيذ جلسات توعوية فورية + تدقيق موقعي هذا الأسبوع.'::text
...
CREATE INDEX IF NOT EXISTS idx_hse_incidents_company_date ON public.hse_incidents(company_id, COALESCE(occurred_at, created_at));